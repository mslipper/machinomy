...
describe('EngineMongo', () => {
    ...
    describe('.connect', () => {
        it('connects to the database', () => {
            const stub = sinon.stub(MongoClient, 'connect')
                .callsFake((conn, cb) => setImmediate(cb(null, 'fake client')));
    ...
    describe('close', () => {
        it('closes the connection', () => {
            ...
            const stub = sinon.stub(MongoClient, 'connect')
                .callsFake((conn, cb) => setImmediate(cb(null, { close })));
            return engine.connect()
                .then(() => engine.close())
                .then(() => {
    ...
    describe('drop', () => {
        it('drops the database', () => {
            const dropDatabase = sinon.stub().callsFake((cb) => cb(null));
            const stub = sinon.stub(MongoClient, 'connect')
                .callsFake((conn, cb) => setImmediate(cb(null, { dropDatabase })));
            return engine.connect()
                .then(() => engine.drop())
    ...
    describe('exec', () => {
        it('provides a client to the callback', () => {
            const stub = sinon.stub(MongoClient, 'connect')
                .callsFake((conn, cb) => setImmediate(cb(null, 'fake client')));
            return engine.connect()
                .then(() => engine.exec((client) => expect(client).toEqual('fake client')))
                .then(() => stub.restore());
        });
        it('lazily connects to the database', () => {
            const stub = sinon.stub(MongoClient, 'connect')
                .callsFake((conn, cb) => setImmediate(cb(null, 'fake client')));
            return engine.exec((client) => expect(client).toEqual('fake client'))
                .then(() => expect(stub.callCount).toBe(1))
                .then(() => stub.restore());
        });
    });
...
describe('EngineNedb', () => {
    ...
    beforeEach(() => {
        ...
        return support_1.tmpFileName().then((path) => {
...
describe('EnginePostgres', () => {
    ...
    describe('exec', () => {
        it('returns an instance of the client', () => {
            const connectStub = sinon.stub(PGClient.prototype, 'connect').resolves();
            return engine.connect()
                .then(() => engine.exec((client) => expect(client instanceof PGClient).toBe(true)))
