...
describe('Buy flow', () => {
    ...
    let senderOriginalBalance;
    let receiverOriginalBalance;
    let hubPort;
    let hubInstance;
    let clientInstance;
    let hubServer;
    ...
    describe('subsequent buy', () => {
        let channelId;
        before(() => {
            return hubInstance.channels().then((channels) => {
    ...
    describe('a buy whose total value is more than the channel value', () => {
        ...
        before(() => {
            return clientInstance.buy({
                ...
                gateway: `http://localhost:${hubPort}/machinomy`,
                meta: ''
            }).then((res) => (newChannelId = res.channelId));
        });
        function verifyChan(channels) {
            expect(channels.length).toBe(2);
            const chan = channels.find((chan) => (chan.channelId === newChannelId));
            if (!chan) {
    ...
    describe('claiming a channel', () => {
        ...
        it('marks the channel as closed for the sender', () => {
            return clientInstance.channels().then((channels) => {
                expect(channels.find((chan) => (chan.channelId === firstChannelId))).toBe(undefined);
            });
        });
